...
public abstract class AppTemplate extends Application {
    ...
    private AppGUI                gui;
    private String                workspaceUsed = "workspace"; // initial value
    ...
    public void setWorkspaceUsed(String s){
        this.workspaceUsed = s;
    }
    ...
    public void start(Stage primaryStage) {
        ...
        try {
            if (loadProperties(APP_PROPERTIES_XML) && loadProperties(WORKSPACE_PROPERTIES_XML) && workspaceUsed.equals("workspace") ) {
    ...
    public void renderLvlSel(Stage primaryStage,AppDataComponent d, AppFileComponent f, String gameMode){
            ...
            if (loadProperties(APP_PROPERTIES_XML) && loadProperties(WORKSPACE_PROPERTIES_XML) && workspaceUsed.equals("workspace") ) {
                ...
                gui = (propertyManager.hasProperty(APP_WINDOW_WIDTH) && propertyManager.hasProperty(APP_WINDOW_HEIGHT))
                      ? new AppGUI(primaryStage, propertyManager.getPropertyValue(APP_TITLE.toString()), this,
                                   ...
                                   Integer.parseInt(propertyManager.getPropertyValue(APP_WINDOW_HEIGHT)))
                      : new AppGUI(primaryStage, propertyManager.getPropertyValue(APP_TITLE.toString()), this);
                workspaceComponent = builder.buildWorkspaceComponentlvlsel(gameMode);
                initStylesheet();
                gui.initStyle();
                workspaceComponent.initStyle();
                gui.getFileController().handleNewRequest();
    ...
    public void renderGamePlay(Stage primaryStage,AppDataComponent d, AppFileComponent f, String gameMode, int difficulty){
        ...
        try {
            if (loadProperties(APP_PROPERTIES_XML) && loadProperties(WORKSPACE_PROPERTIES_XML) && workspaceUsed.equals("workspace") ) {
                ...
                gui = (propertyManager.hasProperty(APP_WINDOW_WIDTH) && propertyManager.hasProperty(APP_WINDOW_HEIGHT))
                        ...
                        Integer.parseInt(propertyManager.getPropertyValue(APP_WINDOW_HEIGHT)))
                        : new AppGUI(primaryStage, propertyManager.getPropertyValue(APP_TITLE.toString()), this);
                workspaceComponent = builder.buildWorkspaceComponentGamePlay(gameMode, difficulty);
                initStylesheet();
                gui.initStyle();
                workspaceComponent.initStyle();
                gui.getFileController().handleNewRequest();
